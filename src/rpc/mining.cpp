...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().KDCHeight) {
            // Solve sha256d.
...
// NOTE: Unlike wallet RPC (which use KDC values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in KDC)\n"
