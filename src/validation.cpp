...
const std::string strMessageMagic = "KadiCoin Signed Message:\n";
...
bool static IsKDCHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.KDCHeight;
}

bool IsKDCHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsKDCHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsKDCHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsKDCHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.KDCHeight;
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < UintToArith256(chainParams.GetConsensus().nMinimumChainWork))
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= (int)chainParams.GetConsensus().KDCHeight)
        return false;
    int64_t target_time = fKDCBootstrapping ? (int64_t)chainParams.GetConsensus().BitcoinPostforkTime : GetTime();
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsKDCHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.KDCHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after KDC fork.
    if (nHeight >= consensusParams.KDCHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.KDCHeight &&
        nHeight < consensusParams.KDCHeight + consensusParams.KDCPremineWindow)
    {
